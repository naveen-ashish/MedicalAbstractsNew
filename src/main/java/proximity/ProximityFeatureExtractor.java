...
public class ProximityFeatureExtractor {
	...
	private HashMap hmMeshTreeDepth = new HashMap();

	private HashMap meshDataAuto = new HashMap();
	...
	/*
	 * @param proxchoice is 1/2/3 depending on whether we want human, animal,
	 * invitro
	 */
	private void generateFeatures(Integer proxchoice) throws Exception {
	...
	private void getUniqueTreeNumbers() {
		...
		for (int i = 0; i < S; ++i) {
			...
			if (parts.length > 1) {
				...
				if (treenum.indexOf(",") > -1)
					treenum = treenum.split(",")[0].trim();
	 ...
	 * 
	 * @return String which is the ancestor of the treenumber but special
	 * handled
	 */
	private String specialCaseTreeNumbers(String treenum, String anctreenum) {
		if (treenum.indexOf("B01.050.150.900.649.801.400.112.400.400") > -1)
			anctreenum = "B01.050.150.900.649.801.400.112.400.400";

		if (treenum.indexOf("F01.145.113") > -1)
			anctreenum = "F01.145.113";
	 ...
	 */
	private int getTotalDepth(String treeNo) {
	...
	private String getAncestor(String treenum) {

		if (treenum.trim().equals(""))
			return null;
	...
	private int getDepth(String node, String anc) {
	...
	public void execute(io.ReadResources myRR) {
	...
	public static void main(String[] args) {
		...
		ProximityFeatureExtractor extractor = new ProximityFeatureExtractor();
		extractor.readtrees();

		extractor.getUniqueTreeNumbers();

		try {
			// generateFeatures(1);

			extractor.generateFeatures(2);
		} catch (Exception e) {
			e.printStackTrace();
		}
